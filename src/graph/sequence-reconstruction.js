...
export const sequenceReconstructionDFS = (org, seqs) => {
  ...
  allTopologicalSortDFS(adjList, indegree, visited, results, []);

  return results.length === 1 && results[0].toString() === org.toString();
};

const allTopologicalSortDFS = (adjList, indegree, visited, results, solution) => {
  ...
  for (let i = 0; i < verticies.length; i++) {
    ...
    if (indegree.get(u) === 0 && !visited.has(u)) {
      ...
      allTopologicalSortDFS(adjList, indegree, visited, results, solution);
